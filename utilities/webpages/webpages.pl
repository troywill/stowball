...
my $query = qq(SELECT packages.id, packages.short_name, webpages.homepage_url FROM webpages INNER JOIN packages ON webpages.id = packages.id);
...
while (my ($id, $short_name, $homepage_url ) = $sth->fetchrow_array) {
    ...
    print HTML "<TR><TD><a href='$homepage_url'>$homepage_url</a></TD></TR>\n";
}
print HTML "</TABLE>\n";
